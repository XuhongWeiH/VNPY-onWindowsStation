...
def run_load_csv():
    ...
    for root, dirs, files in os.walk(csv_root_path, topdown=False):
        try:
            ...
            continue
        if symbol_filter == "AP1810": qidong = True #db.getCollection('db_bar_data').distinct( "symbol", {"interval":"1m"})
        if not qidong : 
...
def csv_load(file, name, mode):
    ...
    with open(file, "r") as f:
        ...
        if mode == 'd':
            ...
            collection_name = re.sub(reg, '', symbol)
            ...
            database_manager.save_bar_data(bars, "Daily_K") 
            database_manager.save_bar_data(bars, "d_"+symbol) 
        #****************************************************************#   
        elif mode == '1m' :
            file_message = name.split('_')
            symbol = file_message[0]
            ...
            database_manager.save_bar_data(bars, "m_"+symbol)
